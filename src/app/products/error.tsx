...
export default function ProductsError({ error, reset }: ProductsErrorProps) {
  useEffect(() => {
    console.error("[Products Error Boundary]", error.message, error.digest);
  }, [error]);
  ...
  return (
    <main className="min-h-screen bg-[#f2f2f2] flex items-center justify-center px-4">
      <div className="text-center max-w-md">
        <div className="mb-6">
          <div className="w-20 h-20 mx-auto rounded-2xl bg-[#fff0f6] flex items-center justify-center mb-4">
            <AlertTriangle className="w-10 h-10 text-[#ff4f8b]" />
          </div>
        </div>
        <h1 className="text-3xl font-black text-[#1a1a1a] mb-2">Products Unavailable</h1>
