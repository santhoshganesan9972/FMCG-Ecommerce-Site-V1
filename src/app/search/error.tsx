...
export default function SearchError({ error, reset }: SearchErrorProps) {
  useEffect(() => {
    console.error("[Search Error Boundary]", error.message, error.digest);
  }, [error]);
      ...
      <div className="text-center max-w-md">
        <div className="mb-6">
          <div className="w-20 h-20 mx-auto rounded-2xl bg-[#fff0f6] flex items-center justify-center mb-4">
            <SearchX className="w-10 h-10 text-[#ff4f8b]" />
          </div>
        </div>
        <h1 className="text-3xl font-black text-[#1a1a1a] mb-2">Search Error</h1>
